...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // 初始化9个子控件
        for (int i = 0; i<9; i++) {
            CJPhotoView *photoView = [[CJPhotoView alloc] init];
...
- (void)setPhotos:(NSArray *)photos
{
    ...
    for (int i = 0; i<self.subviews.count; i++) {
        // 取出i位置对应的imageView
        CJPhotoView *photoView = self.subviews[i];
        ...
        if (i < photos.count) {
            ...
            CGFloat photoX = col * (CJPhotoW + CJPhotoMargin);
            CGFloat photoY = row * (CJPhotoH + CJPhotoMargin);
            photoView.frame = CGRectMake(photoX, photoY, CJPhotoW, CJPhotoH);
...
+ (CGSize)photosViewSizeWithPhotosCount:(int)count
{
    ...
    // 高度
    CGFloat photosH = rows * CJPhotoH + (rows - 1) * CJPhotoMargin;
    
    // 总列数
    int cols = (count >= maxColumns) ? maxColumns : count;
    // 宽度
    CGFloat photosW = cols * CJPhotoW + (cols - 1) * CJPhotoMargin;
